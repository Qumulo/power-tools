...
def test_upgrade_2121_302_hpe():
    global qr
    qr.get_path('2.12.1', '3.0.2', is_hpe=True)
    steps = qr.print_qimg_list()
    assert len(steps) == 5
    assert steps[0] == " 1:    2.12.2  |  qimg: qumulo_install_2.12.2.qimg"
    assert steps[1] == " 2:    2.13.0  |  qimg: qumulo_install_hpe_2.13.0.qimg"
    assert steps[2] == " 3:  2.14.0.3  |  qimg: qumulo_install_2.14.0.3.qimg"
    assert steps[4] == " 5:     3.0.2  |  qimg: qumulo_install_3.0.2.qimg"

def test_upgrade_2121_302():
    global qr
    qr.get_path('2.12.1', '3.0.2')
    steps = qr.print_qimg_list()
    assert len(steps) == 5
    assert steps[0] == " 1:    2.12.2  |  qimg: qumulo_install_2.12.2.qimg"
    assert steps[1] == " 2:    2.13.0  |  qimg: qumulo_install_2.13.0.qimg"
    assert steps[2] == " 3:  2.14.0.3  |  qimg: qumulo_install_2.14.0.3.qimg"
    assert steps[4] == " 5:     3.0.2  |  qimg: qumulo_install_3.0.2.qimg"
