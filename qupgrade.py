...
def download_file(qimg, sharepass):
    ...
    rsp = requests.get(
        TRENDS_DOMAIN + "/data/download/%s?access_code=%s" % (qimg, sharepass),
...
class qumulo_release_mgr:
    def __init__(self):
        ...
        sorted_releases = sorted(
            filter(lambda d: d["is_main_release"] == "1", releases),
            key=lambda d: int(d["release_num"]),
        )
        sorted_quarterly_releases = sorted(
            filter(
                lambda d: d["is_main_release"] == "1"
                and self.is_quarterly(d["full_release"]),
                releases,
            ),
            key=lambda d: int(d["release_num"]),
        )
    ...
    def get_qimg_list(self, release_list, is_hpe, is_cloud):
        ...
        for i, release in enumerate(release_list):
            ...
            the_list.append(
                {
                    "release": self.release_list[release]["full_release"],
                    "qimg": "qumulo_upgrade_%s%s.qimg"
                    % (prefix, self.release_list[release]["full_release"]),
                    "size": qimg_size,
                }
            )
        return the_list
    ...
    def get_path(self, start, end, is_hpe=False, is_cloud=False):
        ...
        for k, rel in release_short_list.items():
            ...
            if self.is_quarterly(rel["main_release"]):
                if (
                    self.get_next_q(rel["main_release"]) is not None
                    and self.get_next_q(rel["main_release"]) <= end_num
                ):
                    between_qs = True
                else:
                    between_qs = False
    ...
    def download_qimgs(self, api, sharepass):
        for d in self.final_release_list:
            ...
            file_exists_on_cluster = api.file_exists(
                "%s/%s" % (UPGRADE_PATH, d["qimg"]), d["size"]
            )
            ...
            if file_exists_on_cluster:
                log_print(
                    "qimg already on cluster: %s" % (UPGRADE_PATH + "/" + d["qimg"])
                )
                continue
            elif file_exists_local and not file_exists_on_cluster:
                log_print(
                    "qimg still needs to be copied to cluster: %s"
                    % (UPGRADE_PATH + "/" + d["qimg"])
                )
                pass
            ...
            with open(d["qimg"], "rb") as fr:
                api.rc.fs.write_file(
                    path="%s/%s" % (UPGRADE_PATH, d["qimg"]), data_file=fr
                )
    ...
    def upgrade_cluster(self, to_version, api, sharepass, download_only=False):
        ...
        if download_only:
            ...
            log_print(
                "If you run without --download-only, the following will be installed: "
            )
            self.print_qimg_list()
            return
...
class qumulo_api:
    ...
    def file_exists(self, full_path, size=None):
        try:
            ...
            if size and int(attr["size"]) != size:
                ...
                log_print(
                    "Expected size: %s - size on Qumulo: %s" % (size, attr["size"])
                )
                return False
        except:
            return False
    ...
    def upgrade_arm(self, qimg_path):
        ...
        try:
            self.rc.request(
                "PUT",
                "/v1/upgrade/settings",
                body={"install_path": qimg_path, "target": "UPGRADE_TARGET_ARM"},
            )
        except:
            exc = sys.exc_info()[1]
            time.sleep(20)
    ...
    def upgrade_prepare(self, version, qimg_path):
        log_print(
            "Preparing cluster for upgrade. Cluster will be fully available during this time."
        )
        try:
            self.rc.request(
                "PUT",
                "/v1/upgrade/settings",
                body={"install_path": qimg_path, "target": "UPGRADE_TARGET_PREPARE"},
            )
        except:
            exc = sys.exc_info()[1]
        ...
        while status == "UPGRADE_STATE_PREPARING":
            ...
            time.sleep(17)
            status = self.upgrade_status()
        log_print("Upgrade prepared for: %s - status: %s" % (qimg_path, status))

    def upgrade_v2_prepare(self, version, qimg_path):
        log_print(
            "Preparing cluster for upgrade. Cluster will be fully available during this time."
        )
        try:
    ...
    def upgrade_to(self, version, qimg_path):
        if get_version_num(version) < get_version_num(UPGRADE_V2_CUTOVER):
            upgrade_status = self.upgrade_status
            upgrade_prepare = self.upgrade_prepare
            upgrade_arm = self.upgrade_arm
        else:
            upgrade_status = self.upgrade_v2_status
            upgrade_prepare = self.upgrade_v2_prepare
            upgrade_arm = self.upgrade_v2_arm
        status = upgrade_status()  # formally stored in resp when this was a rc request
        if status == "UPGRADE_STATE_PREPARED":
            upgrade_arm(qimg_path)
            return
        # if resp["state"] != "UPGRADE_STATE_IDLE":
...
def upgrade_cluster():
    ...
    parser.add_argument(
        "--vers",
        required=True,
        help='The Qumulo Core version to upgrade to. Valid values include: a version number (2.10.0), "latest" and "latest_quarterly"',
    )
    parser.add_argument(
        "--download-only",
        default=False,
        help="Do not perform upgrades, Only download qimg files from fileserver",
        action="store_true",
    )
